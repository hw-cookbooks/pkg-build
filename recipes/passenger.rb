  ...
  node['pkg_build']['passenger']['ruby_versions'].each do |r_ver, pass_vers|
    Array(pass_vers).each do |passenger_version|
      pkg_build_isolate "passenger-rb#{r_ver}" do
        ...
        not_if do
          File.exist?(File.join(node['fpm_tng']['package_dir'], "#{PkgBuild::Ruby.gem_name(node, 'libapache2-mod-passenger', r_ver)}-#{passenger_version}.deb")) &&
            File.exist?(File.join(node['fpm_tng']['package_dir'], "#{PkgBuild::Ruby.gem_name(node, 'passenger', r_ver)}-#{passenger_version}.deb"))
        end
      end
    end
  end
